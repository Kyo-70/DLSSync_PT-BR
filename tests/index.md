...
Native platform gates run `.github/workflows/native.yml` on Linux x64 and macOS Intel/Apple Silicon. They exercise Steam discovery and PE/backup libraries, build the pinned verifier, and package the application. `pe-version` tests reject failed verification and ambiguous publisher identities. A compiled package is not proof of game compatibility.
